...
const AtCoderDifficulty = (): JSX.Element => {
    return (
        <article className="main-article">
            ...
            <p>どうせならユーザー名を入れるだけで自動で情報を取得して生レーティングを計算できるようにしたい。</p>
            <p>
            ...
            <p>ここではAtCoderのABCの順位表のページを参考にして、2の累乗を用いたページネーションにすることにした。</p>

            <h4>0を基準とする</h4>
            <p>
                問題を一覧表示するタブでは推定正答率の昇順で表示しているが、これが50%となる問題が含まれるページの番号が0になるように番号を振っている。
                素直に先頭ページの番号を1にするのに比べて基準となるページの番号を覚えずに済むというメリットがある。勿論ページネーションでも常に0を表示することですぐに基準となるページに戻れるようにしている
            </p>
        </article>
    );
};
